...
from ast_augmentation.evaluation import semantic_variation as analyzer
from ast_augmentation.evaluation.methods import METHODS

LOGGER = logging.getLogger(__name__)
SCRIPT_DIR = Path(__file__).resolve().parent
REPO_ROOT = SCRIPT_DIR.parents[1]
DEFAULT_RESULTS_DIR = REPO_ROOT / "results" / "atlas_sql_br" / "augmentation_methods"
...
def paths_for_method(results_dir, method_key):
    ...
    return AnalysisPaths(
        ...
        scores_output_path=results_dir / f"{base_name}_semantic_variation_scores.json",
        report_output_path=results_dir / f"{base_name}_semantic_variation.xlsx",
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "methods with one shared embedding model."
        )
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "SentenceTransformer device such as cuda or cpu; defaults to auto "
            "detection."
        ),
    )
